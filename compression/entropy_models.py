...
class _EntropyCoder:
    ...
    def __init__(self, method,file_path=None):
    ...
    def encode_with_indexes(self, path,*args, **kwargs):
        if self.name == "ans":
            return self._encoder.encode_with_indexes(*args, **kwargs)
        elif self.name == "rangecoder":
            encoder = range_coder.RangeEncoder(path)
            self._encoder = encoder
            # get max and min, put all the data in [0,max-min]
            # get frequency table, save it .
            # encode(data, freq)
            # seems no need to return
            self.length = len(kwargs["data"])
            if self.length == 0:
                self.max, self.min = 0, 0
                cumfreq = [0]  
                data = []
            else:
                self.max,self.min = max(kwargs["data"]),min(kwargs["data"])
                data = [i-self.min for i in kwargs["data"]]
                cumfreq = [0] + [data.count(i) for i in range(self.max-self.min+1)]
                self.cumfreq = [sum(cumfreq[:i+1]) for i in range(len(cumfreq))]
                self._encoder.encode(data,self.cumfreq)
            with open(path+'.pkl',"wb") as f:
                pickle.dump({'min':self.min,'cumfreq':self.cumfreq,'length':self.length},f)
            self._encoder.close()

    def decode_with_indexes(self, path, *args, **kwargs):
        if self.name == "ans":
            return self._decoder.decode_with_indexes(*args, **kwargs)
        elif self.name == "rangecoder":
            # read max and min, put data back
            # read frequency table
            # decode(len(data),freq)
            # return data
            with open(path+'.pkl',"rb") as f:
                data = pickle.load(f)
                min,cumfreq,length = data['min'],data['cumfreq'],data['length']
            if length == 0:
                return []
            else:
                decoder = range_coder.RangeDecoder(path)
                self._decoder = decoder
                data = self._decoder.decode(length,cumfreq)
...
class EntropyModel(nn.Module):
    ...
    def __init__(
        ...
        self.entropy_coder_name = entropy_coder
        if entropy_coder == "ans":
            self.entropy_coder = _EntropyCoder(entropy_coder)
        elif entropy_coder == "rangecoder":
            self.path = path
            self.entropy_coder = _EntropyCoder(entropy_coder,file_path=self.path)
    ...
    def compress(self, inputs, indexes, means=None):
        ...
        if self.entropy_coder_name == "ans":
            strings = []
            ...
            self.entropy_coder.encode_with_indexes(data = symbols.reshape(-1).int().tolist())
            return None
        
    def compress_range(self,inputs,path):
        symbols = self.quantize(inputs, "symbols")
        self.entropy_coder.encode_with_indexes(path=path,data = symbols.reshape(-1).int().tolist())
    ...
    def decompress_range(self,path):
        return self.entropy_coder.decode_with_indexes(path)
...
class EntropyBottleneck(EntropyModel):
    ...
    *tensorflow/compression*. See the original paper and the `tensorflow
    ...
    def compress_range(self,x,path):
        return super().compress_range(x,path)
    ...
    def decompress_range(self,path):
        return super().decompress_range(path)
