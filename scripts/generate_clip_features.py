...
def create(image_list, data_list, save_folder, model, precompute_seg_path=None):
    ...
    for i, img in tqdm(enumerate(image_list), total=len(image_list), desc="Embedding images", leave=False):
        timer += 1
        if precompute_seg_path is not None:
            ...
            seg_path = None
        try:
            img_embed, seg_map = _embed_clip_sam_tiles(img.unsqueeze(0), sam_encoder, model, seg_path)
        except:
    ...
    for i in range(img_embeds.shape[0]):
        ...
        # print("result saved in", save_path)
...
def _embed_clip_sam_tiles(image, sam_encoder, model, precomp_seg_path=None):
...
def _cal_mask_ans(i, mask_video_np):
    mask_ans = []
    for j in range(1,int(mask_video_np.max())+1):
        positions = np.where(mask_video_np == j)
...
def sam_encoder(image, precomp_seg_path=None):
    ...
    # tasks = [(i, mask_video_np[i]) for i in range(len(mask_video_np))]
    # with Pool(processes=4) as pool:
    #     # starmap 会按顺序返回结果，结果的顺序与输入 tasks 的顺序相同
    #     results = pool.starmap(_cal_mask_ans, tasks)
    # mask_all = results
    ...
    def mask2segmap(masks, image):
        ...
        with Pool(processes=4) as pool:
            # 准备参数列表
            args_list = [(mask, image) for mask in masks]
            # 并行处理
            seg_img_list = pool.map(_process_mask, args_list)
    ...
    # keys = ['default', 's', 'm', 'l']
    # task_args = [(masks_default, image), (masks_s, image), (masks_m, image), (masks_l, image)]
    # with Pool(processes=4) as pool:
    #     results = pool.starmap(_mask2segmap, task_args)
    #
    # for i, (seg_imgs, seg_map) in enumerate(results):
    #     key = keys[i]
    #     seg_imgs = (torch.from_numpy(seg_imgs.astype("float32")).permute(0, 3, 1, 2) / 255.0).to("cuda")
    #     seg_images[key] = seg_imgs
    #     seg_maps[key] = seg_map
...
if __name__ == '__main__':
    ...
    if args.dataset_type == 'hypernerf':
        ...
        for cam_dir in tqdm(cam_dirs):
            ...
            # logger.info(f"img_folder: {img_folder}")
            save_folder = os.path.join(args.precompute_seg, cam_dir, 'language_features' if args.precompute_seg is None else args.output_name)
            if os.path.exists(save_folder) and len(os.listdir(save_folder)) == 600:
                print(f"skipping camera {cam_dir}")
                continue
            ...
            segment_path = os.path.join(args.precompute_seg, cam_dir, "sam_features", "video_mask_concat")
            create(imgs, data_list, save_folder, model, segment_path)
    else:
